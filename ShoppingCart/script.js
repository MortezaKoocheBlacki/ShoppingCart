...
// get filtered products by its category from api
...
const shortenTitle = (text) => {
  const splittedTitle = text.split(" ");

  let newTitle = null;

  if (splittedTitle[1] === "-") {
    newTitle = `${splittedTitle[0]} ${splittedTitle[1]} ${splittedTitle[2]}`;
  } else {
    newTitle = `${splittedTitle[0]} ${splittedTitle[1]}`;
  }

  return newTitle;
};

// Start Initially Project
getProducts();
renderCart();
